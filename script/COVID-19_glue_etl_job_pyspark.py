...
ds0 = glueContext.create_dynamic_frame.from_catalog(database="covid19",
                                                    table_name="covid_19_raw_data",
                                                    transformation_ctx="ds0")
ds0 = ds0.select_fields(['Province/State', 'Country/Region', 'Lat',
                         'Long', 'Date', 'Confirmed', 'Deaths', 'Recovered', 'id'])
...
df = df.select(when(df['Province/State'].isin({'NULL', '', 'missing', '--'}), None)
    ...
    df['Recovered_int']).alias('Recovered'),
    when(df['Confirmed_int'].isNull(), 0).otherwise(
        df['Confirmed_int']).alias('Confirmed'),
    when(df['Deaths_int'].isNull(), 0).otherwise(
        df['Deaths_int']).alias('Deaths'),
